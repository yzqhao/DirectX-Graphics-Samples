...
public:
	...
	inline void SetVertexLayout(RHIDefine::ShaderAttribute attr, RHIDefine::DataType dest, byte size)
	{
		...
		{
			printf("vertex layout already exist");
		}
		else
		{
	...
	inline const std::vector<VertexStreamLayout::Layout*> GetVertexLayouts() const
	{
		std::vector<VertexStreamLayout::Layout*> layouts;
