...
private:
	...
	mutable Math::Vector2   m_FlushInterval;//需要填充更新的区间
...
public:
	...
	void Clear();
	void GetVerticesData(std::vector<Math::Vector3>& vertices) const;
	void GetNormalData(std::vector<Math::Vector3>& vertices) const;
	void GetTexUVData(std::vector<Math::Vector2>& vertices) const;
	void GetTangentData(std::vector<Math::Vector3>& vertices) const;
	...
	inline void ForceVertexCount(int cnt) { m_nVertexCount = cnt; }
	...
	inline const Math::Vector2& GetReflushInterval() const;
...
inline void VertexStream::_DoPushVertexData(VertexPusher* pusher, const void* data)
{
	...
	m_FlushInterval = Math::Vector2(0, m_nVertexCount - 1);
}
...
{
	return &m_Buffer[m_nChunkStride * (int)m_FlushInterval.GetX()];
}
...
{
	m_FlushInterval.SetX(begin);
	m_FlushInterval.SetX(end);
	m_nVertexCount = (std::max)((int)m_nVertexCount, (int)m_FlushInterval.GetY() + 1);
}
	
inline const Math::Vector2& VertexStream::GetReflushInterval() const
{
...
{
	return m_FlushInterval.GetX() != m_FlushInterval.GetY();
}
