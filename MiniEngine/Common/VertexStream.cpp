...
void VertexStream::_Reset()
{
	...
	m_nReserveVertexCount = 0;
	m_nVertexCount = 0;
	m_FlushInterval = Math::Vector2(-1, -1);
	m_Buffer.clear();
	m_Layout.Clear();
...
void VertexStream::_SetVertexType(RHIDefine::ShaderAttribute sa, RHIDefine::DataType source, RHIDefine::DataType dest, int count)
{
	...
	{
		printf("vertex slot %d is exist", sa);
	}
	else
	{
		...
		case RHIDefine::DT_FLOAT:
		{
			...
			default: printf("Unkown vertex data type !");
			}
		}break;
		case RHIDefine::DT_HALF_FLOAT:
		{
...
void VertexStream::GetVerticesData(std::vector<Math::Vector3>& vertices) const
{
	...
	for (int i = 0; i < vertexCnt; i++)
	{
		...
		if (dt == RHIDefine::DT_FLOAT)
		{
			...
			Math::Vector3 vertex;
			vertex.SetX(*vertexBuf); vertexBuf++;
			vertex.SetY(*vertexBuf); vertexBuf++;
			vertex.SetZ(*vertexBuf); vertexBuf++;
...
void VertexStream::GetNormalData(std::vector<Math::Vector3>& vertices) const
{
	const VertexStreamLayout::Layout* vertexLayout = m_Layout.GetVertexLayout(RHIDefine::PS_ATTRIBUTE_NORMAL);
	byte offset = vertexLayout->Offset();
	int vertexSize = GetVertexSize();
	RHIDefine::DataType dt = vertexLayout->DataType();
	byte* bufferDat = (byte*)GetBufferData();
...
void VertexStream::GetTexUVData(std::vector<Math::Vector2>& vertices) const
{
	const VertexStreamLayout::Layout* vertexLayout = m_Layout.GetVertexLayout(RHIDefine::PS_ATTRIBUTE_COORDNATE0);
	byte offset = vertexLayout->Offset();
	int vertexSize = GetVertexSize();
	RHIDefine::DataType dt = vertexLayout->DataType();
	byte* bufferDat = (byte*)GetBufferData();
...
void VertexStream::GetTangentData(std::vector<Math::Vector3>& vertices) const
{
	const VertexStreamLayout::Layout* vertexLayout = m_Layout.GetVertexLayout(RHIDefine::PS_ATTRIBUTE_TANGENT);
	byte offset = vertexLayout->Offset();
	int vertexSize = GetVertexSize();
	RHIDefine::DataType dt = vertexLayout->DataType();
	byte* bufferDat = (byte*)GetBufferData();
...
void VertexStream::CopyVertexBuffer(uint vertexcount, uint vertexsize, const byte* buffer)
{
	...
	{
		printf("vertex copy error: size of each vertex is miss match");
	}
}
