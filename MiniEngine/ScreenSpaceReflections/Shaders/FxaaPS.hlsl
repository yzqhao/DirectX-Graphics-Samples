...
{
    float2 in_uv = pin.TexC;

    int2 screen_size;
    in_color.GetDimensions(screen_size.x, screen_size.y);
    float2 uv_step = float2(1.0 / float(screen_size.x), 1.0 / float(screen_size.y));

    float luma_mat[9];
    for (int i = 0; i < 9; i++) {
        luma_mat[i] = RGB2LUMA(in_color.Sample(gsamLinearWrap, in_uv + uv_step * KERNEL_STEP_MAT[i]).xyz);
    }
    ...
    if(luma_range < max(EDGE_THRESHOLD_MIN, luma_max * EDGE_THRESHOLD_MAX)) {
        return in_color.Sample(gsamLinearWrap, in_uv);
    }
    ...
    float2 uv_start = in_uv + 0.5 * step_normal;
    ...
    float delta_luma_pos = RGB2LUMA(in_color.Sample(gsamLinearWrap, uv_pos).rgb) - luma_average_start;
    float delta_luma_neg = RGB2LUMA(in_color.Sample(gsamLinearWrap, uv_neg).rgb) - luma_average_start;
    ...
    if (!reached_both) {
        for(int i = 2; i < STEP_COUNT_MAX; i++){
            if(!reached_pos) delta_luma_pos = RGB2LUMA(in_color.Sample(gsamLinearWrap, uv_pos).rgb) - luma_average_start;
            if(!reached_neg) delta_luma_neg = RGB2LUMA(in_color.Sample(gsamLinearWrap, uv_neg).rgb) - luma_average_start;
    ...
    return in_color.Sample(gsamLinearWrap, in_uv + pixel_offset * step_normal);
}
